    ...
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-header">
                ...
                <a href="{{ URL( app()->getLocale() . '/admin/pricing/create') }}" class="btn btn-primary" id="sample_editable_1_new" style="float: right;">@lang('main.add_new')
                                ...
                                <table id="example" class="table table-striped table-bordered second text-center" style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ Lang::get('pricing.city') }}</th>
                                            <th>{{ Lang::get('pricing.price') }}</th>
                                            <th>{{ Lang::get('pricing.note') }}</th>
                                            <th>{{ Lang::get('main.action') }}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($pricing as $key => $price)
                                            <tr class="delete_{{$price->id}}">
                                                <td>{{$price->id}}</td>
                                                <td>{{$price['city_'.app()->getLocale()]}}</td>
                                                <td>{{$price['price']}}</td>
                                                <td>{{$price['note_'.app()->getLocale()]}}</td>
                                                <td class="@if(Lang::locale() == 'ar') text-left @else text-right @endif">
                                                    <a class="btn btn-success" href="{{URL( app()->getLocale() . '/admin/pricing/' . $price->id . '/edit')}}" style="padding: 5px 10px;">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <span class="delete_this btn btn-danger" data-id="{{$price->id}}" data-type="Pricing" data-url="pricing" style="padding: 5px 10px;">
